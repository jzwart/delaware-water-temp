...
create_cal_files_denali = function(ind_file,
                                   start,
                                   stop,
                                   model_fabric_file,
                                   obs_file,
  ...
  param_groups = as_tibble(calibration_settings$param_groups)
  ...
  run_sntemp(start = (start-1),
             stop = (start-1),
             spinup = F, # create same conditions as the uncalibrated PRMS-SNTemp output (used for pre-training PGDL )

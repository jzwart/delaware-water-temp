...
d100 = feather::read_feather('4_model_for_PGDL/out/drb_subset_real_train_100obs.feather') %>% group_by(date) %>%
  summarise(sum = sum(exp1 == 'train')) %>% mutate(obs = rep('100', nrow(.)))
d50 = feather::read_feather('4_model_for_PGDL/out/drb_subset_real_train_50obs.feather') %>% group_by(date) %>%
  summarise(sum = sum(exp1 == 'train')) %>% mutate(obs = rep('50', nrow(.)))
d10 = feather::read_feather('4_model_for_PGDL/out/drb_subset_real_train_10obs.feather') %>% group_by(date) %>%
  summarise(sum = sum(exp1 == 'train')) %>% mutate(obs = rep('10', nrow(.)))
d2 = feather::read_feather('4_model_for_PGDL/out/drb_subset_real_train_2obs.feather') %>% group_by(date) %>%
...
all = bind_rows(d100, d50, d10, d2) %>% mutate(n_obs = c(rep('100', nrow(d100)),
                                                       rep('50', nrow(d50)),
                                                       rep('10', nrow(d10)),
                                                       rep('2', nrow(d2))))

ggplot(dplyr::filter(all, exp1 == 'train'), aes(x = date, y = temp_C, group = n_obs, color = n_obs)) +
  geom_point() +
  theme_classic() +
  xlab('Date') + ylab('Temp C') + theme(axis.text = element_text(size = 14))

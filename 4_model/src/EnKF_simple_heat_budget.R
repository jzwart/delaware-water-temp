...
#                 gd_config = 'lib/cfg/gd_config.yml'){
  ...
  ind_file = sprintf('4_model/out/simple_heat_budget_%s_to_%s_%sbeta_%salpha.rds.ind',
  ...
  network_map = create_network_map(model_run_loc = model_run_loc) %>%
    dplyr::filter(from_seg_id_nat %in% model_locations$seg_id_nat | to_seg_id_nat %in% model_locations$seg_id_nat) %>%
    # renaming to model_idx rather than seg_id_nat
    mutate(from_seg_id_nat = model_locations$model_idx[match(from_seg_id_nat, model_locations$seg_id_nat)],
           to_seg_id_nat = model_locations$model_idx[match(to_seg_id_nat, model_locations$seg_id_nat)])
  ...
  prms_data = data_for_heat_budget(model_run_loc = model_run_loc,
                                  model_output_file = 'output/stream_temp.out.nsegment',
                                  model_fabric_file = 'GIS/Segments_subset.shp',
                                  sntemp_vars = vars) %>%
    dplyr::filter(model_idx %in% model_locations$model_idx)

  debug_heat_flux = T
  if(debug_heat_flux){
    debug_heat_flux_out = tibble()
    for(t in 2:n_step){
      print(sprintf('Starting model runs for %s', dates[t]))

      cur_prms_data = dplyr::filter(prms_data, date %in% c(dates[t], dates[t-1]))
      ...
      if(t == 2){
        ...
        updated_temp = debug_heat_flux_out$seg_tave_water[debug_heat_flux_out$date == dates[t-1]]
      }
